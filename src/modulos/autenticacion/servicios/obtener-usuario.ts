 ...
 *
 * Versión para navegador (Client Components y hooks): usa el cliente
 * Supabase de navegador. Consulta la sesión con `auth.getUser()`, carga la
 * fila de `usuarios` (sin `pin_operador`) y resuelve los permisos desde
 * `permisos_rol`; para rol `admin` asigna los 10 permisos de `PERMISOS`
