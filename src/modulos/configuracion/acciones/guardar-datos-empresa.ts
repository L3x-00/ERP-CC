...
import { esquemaConfiguracionEmpresa } from '@/modulos/configuracion/validaciones/indice';
import { ejecutarAccionConfiguracion } from './utilidades-acciones';
import { can } from '@/nucleo/autenticacion/verificar-permiso';
import { crearClienteSupabaseAdmin } from '@/nucleo/supabase/admin';

export async function guardarDatosEmpresaAccion(
  entrada: unknown,
): Promise<RespuestaAccion<ConfiguracionSistema>> {
  const analisis = esquemaConfiguracionEmpresa.safeParse(entrada);
  ...
  return ejecutarAccionConfiguracion(
    ...
    { seccion: 'empresa', campos: Object.keys(analisis.data) },
  );
}
