...
/**
 * Guarda la cotización de una oportunidad con semántica de reemplazo total:
 * borra todas las líneas existentes y reinserta las recibidas con `orden`
 * secuencial. Comparte cuerpo con `actualizarCotizacionAccion` (misma
 * semántica); se duplica por la frontera de `'use server'` — un archivo con
 * esa directiva solo puede exportar Server Actions, no helpers compartidos.
 */
export async function crearCotizacionAccion(entrada: unknown): Promise<RespuestaAccion> {
