...
export function calcularCotizacionTecnica(entrada: unknown): CotizacionTecnicaCalculada {
  ...
  if (router) agregar('router', router.costoMaterial,
  ...
  if (fabricacion) agregar('fabricacion', fabricacion.costoMaterial,
  ...
  return { version: 1, entrada: datos, procesos, costoMaterial, costoServicios, costoOtros, costoTotal,
    precioUnitario, precioTotal: redondear(precioUnitario * datos.cantidad), advertencias };
}
